...
window.Engine = {
	...
	new_battle: function() {
		...
		var fcGo = 0;
/*
				this.debug("battleFG:"+battleFG);
					this.debug("ballteCount:"+ballteCount);
						this.debug("ballteCountTotal:"+ballteCountTotal);
							this.debug("turn"+turn);
								this.debug("bossMode"+bossMode);
									this.debug("bossHp"+bossHp);
										this.debug("bossMaxHp:"+bossMaxHp);
											this.debug("bossMaxStar"+bossMaxStar);
												this.debug("bossStar"+bossStar);
													this.debug("bossRecast"+bossRecast);
			...
			if (ballteCount == ballteCountTotal) {
				...
				for (var i = 0; i <= 3; i++) {
					for (var j = 0; j <= 3; j++) {
						...
						//普通技能
						else if (window.localStorage.getItem("skill-" + (i + 1) + "-" + (j + 1)) == '2' && $(".lis-ability:eq(" + c + ")").hasClass("btn-ability-available") && !$(".prt-command-chara:eq(" + i + ")").hasClass("ability-disable")) {
							this.click(".lis-ability:eq(" + c + ")");
							return;
						}
						//BOSS OD时释放的技能
...
'_handle_raid_multi': function() {
	...
	Promise.race([
	this.waitUntilVisible('div.prt-popup-header:contains("救援依頼")'), this.sleep(1)]).then(function() {
